...
# -*- coding: utf-8 -*-
"""
LLM-based baseline QA generation from support facts.

Rules:
- support length = 1: Generate 10 question variants and 10 answer variants, Cartesian product to fill 100.
- support length = x>1: For each fact, generate ceil(sqrt(100/x)) questions and same number of answers,
  do Cartesian product within each fact, then aggregate and truncate/pad to 100.
"""
import re
import argparse
import math
...
def pad_or_truncate(seq: Sequence[Any], target: int) -> List[Any]:
    if not seq:
        return []
    if len(seq) >= target:
...
def dedup_pairs(pairs: Iterable[Tuple[str, str, int]]) -> List[Tuple[str, str, int]]:
    seen = set()
    uniq: List[Tuple[str, str, int]] = []
    for q, a, sidx in pairs:
        key = (q, a)
        if key in seen:
            continue
        seen.add(key)
        uniq.append((q, a, sidx))
    return uniq


def normalize_support(sample: Dict[str, Any]) -> Tuple[List[str], Dict[str, bool]]:
    """
    Extract and flatten support list, compatible with different fields and nesting, 
    and add original_question + original_answer.
    Returns (supports, reason_dict)
    """
    candidates = [
    ...
    # Check if support field is empty
    support_empty = False
    if not support_raw or (isinstance(support_raw, list) and len(support_raw) == 0):
        support_empty = True
    ...
    # Add original_question + original_answer
    ...
    if not orig_qa_empty:
        orig_fact = f"{orig_q} {orig_a}"
        cleaned = " ".join(orig_fact.strip().split())
        if cleaned and cleaned not in seen:
            supports.append(cleaned)
...
def build_pairs_for_support(client: LLMClient, fact: str, qn: int, an: int) -> Tuple[List[Tuple[str, str]], str]:
    questions, answers, raw_text = gen_variants_with_llm(client, fact=fact, n=max(qn, an))
    questions = pad_or_truncate(questions, qn)
    answers = pad_or_truncate(answers, an)
...
def generate_for_sample(
    ...
    x = len(supports)
    results: List[Tuple[str, str, int]] = []  # (q, a, support_idx)
    raw_texts = []
    if x == 1:
        pairs, raw_text = build_pairs_for_support(client, supports[0], 10, 10)
        for q, a in pairs:
            results.append((q, a, 0))
        raw_texts.append(raw_text)
    else:
        per_count = math.ceil(math.sqrt(TARGET_COUNT / x))
        for idx, fact in enumerate(supports):
            pairs, raw_text = build_pairs_for_support(client, fact, per_count, per_count)
            for q, a in pairs:
                results.append((q, a, idx))
            raw_texts.append(raw_text)

    if dedup:
        results = dedup_pairs(results)

    results = pad_or_truncate(results, TARGET_COUNT)

    formatted: List[Dict[str, Any]] = []
    for q, a, sidx in results:
        item = {"question": q, "answer": a, "raw_text": raw_texts[sidx]}
        if include_meta:
            item["source_id"] = source_id
            item["support_idx"] = sidx
...
def process(
    ...
    def task(idx_sample: Tuple[int, Dict[str, Any]]) -> Tuple[int, List[Dict[str, Any]], bool, Dict[str, bool]]:
        ...
        try:
            supports, reason = normalize_support(sample)
        ...
        qa_list = generate_for_sample(client, supports, source_id, include_meta, dedup)
        ...
        for fut in tqdm(as_completed(futures), total=total, desc="Generating baseline QA", unit="sample"):
            ...
            if is_skipped:
                ...
                if reason.get("format_error"):
                    ...
                    if reason.get("support_empty"):
                        skip_stats["support_empty"] += 1
                    ...
                    if reason.get("all_empty"):
                        skip_stats["all_empty"] += 1
                skipped += 1
                continue
            all_outputs.extend(qa_list)

    print(f"Processed {len(data)} samples; skipped {skipped} without support.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LLM-based baseline QA generation from support facts.")
    ...
    parser.add_argument("--include_meta", action="store_true", help="Include source_id and support_idx in output.")
