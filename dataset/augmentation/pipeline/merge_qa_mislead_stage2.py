...
STAGE1_PATH = "/disk0/xuhaoming/confidence/dataset/augmentation/result/train_dataset_distill+oqnqshortqa_stage1.json"
C4_PATH = "/disk0/xuhaoming/confidence/c4_en_500.json"
OUTPUT_PATH = "/disk0/xuhaoming/confidence/dataset/augmentation/result/train_dataset_distill+nqqa+c4qa_longqa_stage2.json"
...
def main():
    random.seed(42)
    stage1_items = load_json(STAGE1_PATH)
    c4_items = load_json(C4_PATH)
    c4_texts = []
    for x in c4_items:
        s = x.get("text")
        if isinstance(s, str) and s.strip():
